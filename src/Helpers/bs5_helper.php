...
if (!function_exists('bs5_cardheader')) {
    ...
    function bs5_cardheader($data)
    {
        ...
        <div class="card-header">
            <i class="' . $data['icon'] . ' fa-lg me-2"></i><strong>' . $data['title'] . '</strong>
            <a href="' . $data['help'] . '" target="_blank" class="float-end card-header-help-icon" title="' . lang('Auth.getHelpForPage') . '"><i class="far fa-question-circle fa-lg"></i></a>
        </div>';
...
if (!function_exists('bs5_searchform')) {
    ...
    function bs5_searchform($action, $search)
    {
        ...
        <form class="input-group" name="form_search" action="' . $action . '" method="post">
            ...
            <button class="btn btn-outline-secondary" type="submit" name="btn_reset" id="btn_reset" title="' . lang('Auth.btn.reset') . '..."' . $disabled . '><i class="fas fa-backspace fa-lg text-danger"></i></button>
            <button class="btn btn-outline-secondary" type="submit" name="btn_search" id="btn_search" title="' . lang('Auth.btn.search') . '..."><i class="fas fa-search fa-lg text-primary"></i></button>
        </form>';
